...
- `docs/known-issues.md`：所有**未修复缺陷（bug）**（前端/后端/驱动），每条带最小复现与临时绕过；
  **修完即删**。其他任务过程中发现、无法当场修复的 **bug** 必须补录到本文件；
  功能缺口/设计取舍/优化/技术债等非 bug 事项走 plans。
- `docs/plans.md`：未来规划与待办事项（功能/优化/重构/技术债/测试等），**完成即删**；
  新发现的未来事项补录到对应分类。
