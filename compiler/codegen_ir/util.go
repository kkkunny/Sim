...
func (self *CodeGenerator) buildEqual(t hir.Type, l, r llvm.Value, not bool) llvm.Value {
	...
	case *hir.TupleType, *hir.StructType:
		...
		fields := stlval.TernaryAction(isTuple, func() []hir.Type {
			return irType.(*hir.TupleType).Elems
		}, func() []hir.Type {
			values := irType.(*hir.StructType).Fields.Values()
			res := make([]hir.Type, len(values))
	...
	case *hir.EnumType:
		...
		indexBlockPairs := stlslices.Map(irType.Fields.Values(), func(i int, f hir.EnumField) struct {
			Value llvm.Value
			Block llvm.Block
		} {
			if f.Elem.IsNone() {
...
func (self *CodeGenerator) buildCopy(t hir.Type, v llvm.Value) llvm.Value {
	...
	case *hir.TupleType, *hir.StructType:
		...
		fields := stlval.TernaryAction(stlval.Is[*hir.TupleType](tir), func() []hir.Type {
	...
	case *hir.EnumType:
		...
		indexBlockPairs := stlslices.Map(tir.Fields.Values(), func(i int, f hir.EnumField) struct {
			Value llvm.Value
			Block llvm.Block
		} {
			if f.Elem.IsNone() {
