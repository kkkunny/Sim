...
- **单项完成即从本文件删除**（不保留已完成条目；历史见 git log），删除后其后条目编号顺延重排。
- **本清单按推荐完成的先后顺序排列，编号即顺序**；其他任务过程中发现的、未来需要完成的事项，
  按推荐时机插入清单中的相应位置（新事项不必然追加到末尾），插入点之后的条目编号顺延重排。
- 排序参考：先安全网（测试）与低成本高收益项（语法缺口、卫生清理），再健壮性与功能补齐，
  随后性能与重构（有依赖的排在依赖项之后），最后长期设计（如内存管理）。
- 条目包含：目标 / 现状与动机 / 验收方向（足够开工即可）；每条标注分类
  （性能与构建 / 架构与设计 / 工程与质量 / 语言与运行时）。
- 编号会随插入/重排变动；跨文档或提交信息引用条目时请带上标题。
...
### P10. bind（方法）在 HIR lowering 统一 desugar
